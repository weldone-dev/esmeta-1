...
sealed trait MathTy extends TyElem with Lattice[MathTy] {
  ...
  /**
   * Partial order. 
   * In default, IntSignTy is a subset of the MathSignTy.
   * We cannot ensure other orders, unless we can directly compare them if some of them are SetTy.
   */
...
case class MathSetTy(set: Set[Math]) extends MathTy {
  ...
  def toSign: MathSignTy = MathSignTy(Sign.alpha(set, 
    v => if v.decimal < 0 then Sign.Neg else if v.decimal > 0 then Sign.Pos else Sign.Zero))
}
